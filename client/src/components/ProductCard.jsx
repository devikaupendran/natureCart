...
const ProductCard = ({ product }) => {
    ...
    const { currencySymbol, addToCart, cartItems, removeCartItem, navigate } = useAppContext();

    //if the product data available then only show this container
    return product && (
        <div className="border border-gray-200 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full">
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div className='text-green-800' onClick={(e) => { e.stopPropagation() }}>
                        {
                            !cartItems[product._id] ? (
                                <button className="flex items-center justify-center gap-1 bg-green-100 border border-green-300 md:w-[80px] w-[64px] h-[34px] rounded cursor-pointer"
                                    onClick={() => addToCart(product._id)} >
                                    <img src={assets.cart_icon} alt="cart" />
                                    Add
                                </button>
                            ) : (
                                <div className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-green-600 rounded select-none text-white">
